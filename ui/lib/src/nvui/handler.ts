...
type Commands = {
  moveLeft: (isWhite: boolean, currentActiveSquare: HTMLButtonElement, borderSound: () => void) => void;
  moveRight: (isWhite: boolean, currentActiveSquare: HTMLButtonElement, borderSound: () => void) => void;
  moveDown: (isWhite: boolean, currentActiveSquare: HTMLButtonElement, borderSound: () => void) => void;
  moveUp: (isWhite: boolean, currentActiveSquare: HTMLButtonElement, borderSound: () => void) => void;
  announceLastMove: () => void;
  announceTime: () => void;
  announceSquare: (currentActiveSquare: HTMLButtonElement) => void;
  jumpToRank: (currentActiveSquare: HTMLButtonElement, rank: string) => void;
  jumpToFile: (currentActiveSquare: HTMLButtonElement, file: string) => void;
  announcePossibleCaptures: (
    yourColor: Color,
    cg: CgApi,
    variant: VariantKey,
    steps: RoundStep[],
...
export const AdvancedBlindModeCommands: Commands = {
  ...
  jumpToRank: (currentActiveSquare: HTMLButtonElement, rank: string) => {
    const key = keyFromAttrs(currentActiveSquare as HTMLElement);
    if (!key) return;
    const file = key[0];
    const newSqEl = document.querySelector<HTMLElement>(squareSelector(rank, file));
    newSqEl && newSqEl.focus();
  },
  jumpToFile: (currentActiveSquare: HTMLButtonElement, file: string) => {
    const key = keyFromAttrs(currentActiveSquare as HTMLElement);
    if (!key) return;
    const rank = key[1];
    const newSqEl = document.querySelector<HTMLElement>(squareSelector(rank, file));
  ...
  jumpToPiece: (
    ...
    const myBtnAttrs = squareSelector(
      currentActiveSquare.getAttribute('rank') ?? '',
      currentActiveSquare.getAttribute('file') ?? '',
    );
    const $allPieces = $(`.board-wrapper [piece="${piece.toLowerCase()}"], ${myBtnAttrs}`);
    const myPieceIndex = $allPieces.index(myBtnAttrs);
    const $prevNextPieces = reverse ? $allPieces.slice(myPieceIndex + 1) : $allPieces.slice(0, myPieceIndex);
    const pieceEl = reverse ? $prevNextPieces.get(0) : $prevNextPieces.get($prevNextPieces.length - 1);
    if (pieceEl) pieceEl.focus();
    // if detected any matching piece; one is the piece being clicked on,
    else if ($allPieces.length >= 2) {
      const wrapPieceEl = reverse ? $allPieces.get(0) : $allPieces.get($allPieces.length - 1);
      wrapPieceEl?.focus();
      selectSound();
    } else errorSound();
  },
...
export class RegexLexer {
  ...
  registerToken(token: { name: string; regex: string }): void {
    if (this.isCompiled) throw new Error('Cannot register token after compilation');
...
export interface RegexParserKeyboardActionsType {
  name: string;
  regex: string;
...
export class RegexParserKeyboardEventHandler {
  ...
  registerInputPatterns(patterns: RegexParserKeyboardActionsType[]): RegexParserKeyboardEventHandler {
    patterns.forEach(pattern => this.registerInputPattern(pattern));
    return this;
  }
